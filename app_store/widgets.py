...
class ShopLinkWidget(LinkWidget):
    ...
    def option_string(self):
        link = (
            '<a%(attrs)s %(classes)s id="%(id)s" href="?%(query_string)s">%(label)s</a>'
        )
        return link
    ...
    def render_option(self, name, selected_choices, option_value, option_label):
        ...
        if (option_value in selected_choices) or (
            "" in selected_choices and option_value == "price"
        ):
        ...
        if "" not in selected_choices:
            if (
                ...
                or f"-{option_value}" in selected_choices
            ):
